  ...
  const appScreens = [
    {
      ...
      caption: "Anzahl Tassen, Zielerreichung und Koffeinkonsum auf einen Blick"
    },
    {
      image: "/images/dashboard-screen.png", 
      title: "Ziele & Challenges",
      caption: "100 Tassen in 30 Tagen – mit Countdown und Motivation"
    },
    {
      // Reusing statistics screen as placeholder for the reward screen
      image: "/images/statistics-screen.png", 
      title: "Reward-Übersicht",
      caption: "Belohnungen bei Partnercafés mit QR-Code zum Einlösen"
    }
  ];

  return (
    <section id="app-highlights" className="w-full py-16 md:py-24 px-6 md:px-16 bg-gradient-to-b from-amber-100/40 to-amber-50/80">
                ...
                {appScreens.map((screen, index) => (
                  <div key={index} className="min-w-full flex flex-col items-center">
                    <div className="relative mb-4">
                      <div className="absolute inset-0 bg-[#6F4E37] rounded-3xl opacity-10 blur-md transform -rotate-1"></div>
                      <Image
                        src={screen.image}
                        alt={screen.title}
                        width={280}
          ...
          <div className="relative">
            <div className="flex justify-center items-center mb-16">
              {appScreens.map((screen, index) => (
                <div 
                  key={index} 
                  className="relative mx-6 transform transition-all duration-300 hover:scale-105"
                  style={{ 
                    transform: index === 0 ? 'perspective(1000px) rotateY(15deg)' : 
                             index === 2 ? 'perspective(1000px) rotateY(-15deg)' : 
